...
> => {
  const appFileBase = casing.fileBase("app");
  const mainFileBase = casing.fileBase("main");
  const appConfigFileBase = casing.fileBase("app_config");
  const appDir = casing.directory("app");
  return {
    vite: {
      scaffold: [
        ...
        {
          filename: imports.frontend(`src/${mainFileBase}.tsx`),
          tmpl: vite.mainTsx,
          kind: "content",
        },
        {
          filename: imports.frontend(`src/${appFileBase}.tsx`),
          tmpl: vite.appTsx,
          kind: "content",
        },
    ...
    svelte: {
      scaffold: [
        ...
        {
          filename: imports.frontend(`src/${mainFileBase}.ts`),
          tmpl: svelte.mainTs,
          kind: "content",
        },
        {
          filename: imports.frontend(`src/${appFileBase}.svelte`),
          tmpl: svelte.appSvelte,
          kind: "content",
        },
    ...
    angular: {
      scaffold: [
        ...
        {
          filename: imports.frontend(`src/${appDir}/${appConfigFileBase}.ts`),
          tmpl: angular.appConfigTs,
          kind: "content",
        },
        {
          filename: imports.frontend(`src/${appDir}/${appFileBase}.ts`),
          tmpl: angular.appTs,
          kind: "content",
        },
...
class Generator extends Emit {
  from(): GenerateEntry[] {
    const tokens = {
      application_name: this.settings.applicationName,
      appComponentName: this.casing.appComponentName(),
      homeComponentName: this.casing.homeComponentName(),
      rootLayoutName: this.casing.rootLayoutName(),
      appConfigName: this.casing.appConfigName(),
      appFileBase: this.casing.fileBase("app"),
      mainFileBase: this.casing.fileBase("main"),
      appConfigFileBase: this.casing.fileBase("app_config"),
      appDir: this.casing.directory("app"),
      mainFile: `${this.casing.fileBase("main")}.ts`,
    };
    const { scaffold, fullStack } = frameworks(this.imports, this.casing)[
