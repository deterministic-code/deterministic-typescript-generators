...
import type {
  ...
  NamedField,
  ResolvedSpec,
} from "@deterministic-code/generator-sdk/codegen/lib/service-integration-tests-emit-types";
import {
...
/** The id-representation knobs an emitted test derives from a `DatasourceSettings`: the TS type of `id`, whether a separate `uuid` column exists, a sentinel id no row will hold (for the missing-row branches, quoted as a TS literal), and the repository option fields that make `.add()` honor a uuid primary key. */
...
/** The module-level `PrimaryKeyService` the emitted test injects into every repository it builds: one registration per entity it constructs (the entity under test plus each seeded parent table), each keyed on the same name passed as `options.entityName`. All standard id-based repos, so the PK column is always `id`. */
...
/** The library-import specifier for `@deterministic-code/deterministic/repositories`, resolved for the emitted test's on-disk location (feature dir when `organizeByFeature`, else the flat `services/generated/__tests__` layout). */
...
export function emitGenericServiceIntegrationTest(
  ...
  opts: TsEmitOptions = DEFAULT_EMIT_OPTIONS,
): EmittedFile {
  const {
    entityName,
    className,
