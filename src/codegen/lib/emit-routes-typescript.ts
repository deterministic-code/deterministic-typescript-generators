...
import {
  emitNestedDirectFkRouter,
  emitNestedM2mRouter,
} from "./emit-nested-routes-typescript.ts";
import {
  snakeToCamel,
  snakeToKebab,
  camelPlural,
...
export function routeServiceImport(
  ...
  // lookup key into a Set of kebab feature dirs (ImportPaths.featureEntity output); not an emitted name.
  const targetSubdir = customServiceEntities.has(kebab(targetEntity)) // lint-emitter-casing-allow: kebab
    ? "custom"
    : undefined;
...
function routeFilePlan(
  ...
  opts: RouteEmitOptions,
): { emitPath: string; fileRelPath: string } {
  const byFeature = opts.organizeByFeature === true;
  const fileBase = namesFor(opts).fileBase(candidate.name, "route");
  const emitPath = layoutFor(opts).filePath(candidate.name, "route");
  const fileRelPath = byFeature ? emitPath : `routes/generated/${fileBase}.ts`;
  return { emitPath, fileRelPath };
}
...
function routeHead(
  ...
  const { emitPath, fileRelPath } = routeFilePlan(candidate, opts);
  const names = namesFor(opts);
  return {
    entity: names.className(candidate.name),
    fnName: `${camelPlural(candidate.name)}Router`, // lint-emitter-casing-allow: camelPlural
...
function byFieldRouteCode(
  ...
  const fieldCamel = snakeToCamel(byField); // lint-emitter-casing-allow: snakeToCamel
  const fieldKebab = snakeToKebab(byField); // lint-emitter-casing-allow: snakeToKebab
...
function readOnlyPlain(
  ...
  const { entity, fnName, emitPath, serviceImport, libImports } = head;
...
function byFeatureRoutePath({
  ...
  if (parts[0] !== "features") {
    ...
    throw new Error(
      `emitCustomRouteStub: route "${name}" has module "${mod}" which is outside ./features/. ` +
...
// Mirrors resolveCustomEmitPath in services emitter. Under by-feature the runtime's loadClass() must find the stub at exactly the path emitted, so the emitted app.ts hands createBackendApp a customModulePaths entry pointing at the convention path when the user didn't set one.
...
export function emitCustomRouteStub(
  entry: RouteEntry,
  options: RouteEmitOptions & { byFeature?: boolean } = {},
): EmittedFile {
...
function groupSiblingsByDir(files: EmittedFile[]): Map<string, IndexSibling[]> {
...
export function emitIndexFromFiles(files: EmittedFile[]): EmittedFile[] {
...
function appWiringPlacement(byFeature: boolean): {
  ...
  return byFeature
    ? {
        emitPath: "features/app-wiring.ts",
        fileRelPath: "features/app-wiring.ts",
      }
    : {
        emitPath: "app-wiring.ts",
...
export function emitAppWiring(
  ...
  const { emitPath, fileRelPath } = appWiringPlacement(byFeature);
  ...
  const imports = wiring.routers.map((r) => {
    const fnName = `${camelPlural(r.name)}Router`; // lint-emitter-casing-allow: camelPlural
    ...
    emitRoutesFiles({
      ...
      primitives: {
        ...
        nestedRouterEmitters: {
          "direct-fk": emitNestedDirectFkRouter,
          m2m: emitNestedM2mRouter,
        },
      },
    }),
});
