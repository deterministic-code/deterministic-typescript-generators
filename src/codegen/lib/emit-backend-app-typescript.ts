...
import { resolveCustomEmitPath } from "./emit-services-typescript.ts";
import { resolveCustomRoutePath } from "./emit-routes-typescript.ts";
import { emitTestApp } from "./emit-test-app-typescript.ts";
...
function toSpecifier(emitPath: string): string {
...
export function buildCustomModulePaths({
  ...
  for (const entry of asArray(servicesDoc?.services)) {
    ...
    if (mod) {
      const path = resolveCustomEmitPath(
        entry as Parameters<typeof resolveCustomEmitPath>[0],
        names,
        true,
      );
...
async function bundledDistEntries(): Promise<EmitEntry[]> {
...
async function typescriptScaffoldEntries({
  ...
  laneDir,
  organizeByFeature,
}: ScaffoldEntriesInput): Promise<EmitEntry[]> {
...
async function typescriptTestEntries({
  ...
  settings,
  libraryReferenceMode,
}: TestEntriesInput): Promise<EmitEntry[]> {
