...
/** The `{ inputs, settings }` argument every bindings-driven frontend emitter's `emit` receives — `inputs` is the untyped codegen input adapter, `settings` the resolved settings the naming bridges read. */
export type EmitArgs = { inputs: unknown; settings: Settings };

/** An OpenAPI `components/schemas` node as the frontend emitters read it: a scalar (`type`/`format`), a `$ref`, a `oneOf` union, an `array` with `items`, or an object with `properties`. Validators additionally read `required`, `maxLength`, and `x-references`; every field is optional so both the types and validators lanes share one shape. */
