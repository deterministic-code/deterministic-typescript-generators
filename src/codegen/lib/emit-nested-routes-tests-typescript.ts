...
function nestedIdFactory(options: NestedRouteTestOptions) {
...
function primaryKeyMock(
  options: NestedRouteTestOptions,
  testPath: string,
): PrimaryKeyMock {
...
function nestedM2mTestContent(
  ...
  const parentPascal = snakeToPascal(descriptor.parent); // lint-emitter-casing-allow: snakeToPascal
  const targetPascal = snakeToPascal(descriptor.target); // lint-emitter-casing-allow: snakeToPascal
...
function nestedDirectFkTestContent(
  ...
  const childPascal = snakeToPascal(childName); // lint-emitter-casing-allow: snakeToPascal
  const validatorFileBase = namesFor(options).fileBase(
    childName,
    "view-validator",
  );
  const validatorPath = layoutFor(options).testImportSpecifier(
    { entity: nestedRouteEntity(descriptor), artifact: "route" },
    { entity: childName, artifact: "view-validator" },
    {
...
/** By-feature-aware emitted path for a nested router test (`features/<entity>/__tests__/<plural>.integration.test.ts`). */
function nestedRouterTestPath(
...
export function emitNestedDirectFkRouterTest(
  descriptor: NestedRouteDescriptor,
  options: NestedRouteTestOptions = {},
): EmittedFile {
  if (descriptor.kind !== "direct-fk") {
    throw new Error(
      `emitNestedDirectFkRouterTest: expected direct-fk descriptor, got ${descriptor.kind}`,
    );
  }
  return {
...
export function emitNestedM2mRouterTest(
  descriptor: NestedRouteDescriptor,
  options: NestedRouteTestOptions = {},
): EmittedFile {
  if (descriptor.kind !== "m2m") {
    throw new Error(
      `emitNestedM2mRouterTest: expected m2m descriptor, got ${descriptor.kind}`,
    );
  }
  return {
    path: nestedRouterTestPath(descriptor, options),
