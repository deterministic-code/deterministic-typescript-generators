...
import {
  emitNestedDirectFkRouterTest,
  emitNestedM2mRouterTest,
} from "./emit-nested-routes-tests-typescript.ts";
...
export const DEFAULT_EMIT_OPTIONS: TestEmitOptions = {
  schemaVersion: "1.0",
  apiBase: "/api",
  fileFormat: "Camel",
};
...
export function emitReadOnlyRouterTest(
  candidate: TestCandidate,
  opts: TestEmitOptions = DEFAULT_EMIT_OPTIONS,
): EmittedFile {
...
export function emitCombinedRouteTests(
  {
    routesData,
    datasourceData,
  }: { routesData: unknown; datasourceData: unknown },
  emitOptions: TestEmitOptions,
): EmittedFile[] {
