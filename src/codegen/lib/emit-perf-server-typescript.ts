...
export function emitPerfServerTypescript({
  datasourceData,
  libraryReferenceMode,
}: PerfServerTsOptions = {}): EmittedFile {
  if (!datasourceData || typeof datasourceData !== "object") {
    throw new Error(
      "emitPerfServerTypescript: datasourceData is required (parsed datasource_types.yaml)",
    );
  }
  if (!Array.isArray(datasourceData.types)) {
    throw new Error(
      "emitPerfServerTypescript: datasourceData.types must be an array — malformed datasource_types.yaml",
    );
  }
...
export async function emit({ inputs, settings }: PerfServerTsEmitInput) {
