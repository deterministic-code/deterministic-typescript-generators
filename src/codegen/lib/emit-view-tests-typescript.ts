...
export type EmitOptions = Flatten<
  NamesForOptions & {
...
function fileBase(name: string, opts: EmitOptions): string {
...
function renderShapedView(args: ShapedRenderArgs): EmittedFile {
...
export function emitForView({
  ...
  const opts: EmitOptions = { ...DEFAULT_EMIT_OPTIONS, ...options };
  if (view.kind === "union") {
...
export function emitFromSchema(
  ...
  options: Partial<EmitOptions> = DEFAULT_EMIT_OPTIONS,
): EmittedFile[] {
  const opts: EmitOptions = { ...DEFAULT_EMIT_OPTIONS, ...options };
  const normalized = normalizeAll(viewTypes as RawTypesDoc) as View[];
  return normalized.map((v) =>
    emitForView({ view: v, viewTypes, datasource, options: opts }),
  );
}
