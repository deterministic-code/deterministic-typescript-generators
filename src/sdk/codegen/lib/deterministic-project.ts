...
/** The dialect axis for a datasource SQL step: the project-wide `backend.datasources` list from settings, else a single default dialect. */
export function resolveDatasourceDialects(settings: SettingsDict): string[] {
  const datasources = settingsList(settings, "backend.datasources");
  return datasources.length > 0 ? datasources : [DEFAULT_SQL_DIALECT];
}
...
export function resolveSchemaDialects(settings: SettingsDict): string[] {
  return withSqliteDialect(resolveDatasourceDialects(settings));
}
