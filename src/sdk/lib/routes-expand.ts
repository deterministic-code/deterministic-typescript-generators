...
function transformEntityTail(entity: string, multiplicity: string): string {
  ...
  const finalLower = multiplicity === "many" ? pluralize(singular) : singular;
...
function expandByFieldRoute(
  ...
  const collection = `/api/${kebabPlural(entity)}/${snakeToKebab(byField)}`;
  const memberPath = `${collection}/:${snakeToCamel(byField)}`;
  ...
  return byFieldMethodRoutes(
    {
      camel: snakeToCamel(entity),
      byFieldPascal: snakeToPascal(byField),
      memberPath,
      entity,
      meta,
    },
...
function buildCrudRoutes(o: BuildCrudOptions): RouteEntry[] {
  const camel = snakeToCamel(o.entity);
...
function segmentPascal(segmentTail: string): string {
  return snakeToPascal(segmentTail.replace(/-/g, "_"));
}
...
function combinedRoutePrefix(descriptor: {
  ...
  return (
    snakeToCamel(descriptor.parent) + segmentPascal(descriptor.segmentTail)
  );
}
