...
/** DB-wiring marker-block PATCH entries into the sibling backend scaffold (app.ts / entrypoint). Empty blocks (already-empty template sections) are dropped — patch entries require non-empty content. */
...
/** Every migrate target (runner files + app.ts/entrypoint/package.json/.env/.gitignore/Dockerfile) is generated by tsEntries as a CONTENT or PATCH entry — no sibling-patching `patch()` remains. */
