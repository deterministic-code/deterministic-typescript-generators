...
import { emitNestedDirectFkRouterTest, emitNestedM2mRouterTest, } from "./emit-nested-routes-tests-typescript.js";
...
function routerTestNaming(candidate, opts) {
    ...
    const apiBase = opts.apiBase ?? DEFAULT_EMIT_OPTIONS.apiBase;
    return {
...
export function emitReadOnlyRouterTest(candidate, opts = DEFAULT_EMIT_OPTIONS) {
...
export function emitCrudRouterTest(candidate, opts = DEFAULT_EMIT_OPTIONS) {
...
export function emitCombinedRouteTests({ routesData, datasourceData, }, emitOptions) {
    const files = [];
    ...
    for (const d of descriptors) {
        if (d.kind === "direct-fk") {
            files.push(emitNestedDirectFkRouterTest(d, emitOptions));
        }
        else if (d.kind === "m2m") {
            files.push(emitNestedM2mRouterTest(d, emitOptions));
        }
    }
    return files;
}
