...
export function buildOpenApiDocFromData({ routesData, viewData, datasourceData, settings, overrides = {}, }) {
    ...
    return buildEnrichedOpenApiSpec(buildArgs);
}
...
export async function buildOpenApiDocFromInputs({ inputs, settings, }) {
    const { routesYamlText, viewYamlText, datasourceYamlText, datasourceSeedsYamlText, } = await inputs.all();
    return buildOpenApiDocFromData({
        routesData: parse(routesYamlText),
        viewData: parse(viewYamlText),
        datasourceData: parseDatasourceTypes(datasourceYamlText, settings, datasourceSeedsYamlText),
        settings,
    });
}
