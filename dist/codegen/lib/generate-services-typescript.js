...
function buildTsFinders(byFields, ctx) {
...
function typeImportLine(candidate, layout, ctx) {
    const artifact = candidate.kind === "datasource_type" ? "datasource-type" : "view-type";
    const typeImportPath = layout.importSpecifier({ entity: candidate.name, artifact: "service" }, { entity: candidate.name, artifact });
    return `import { ${ctx.typeName} } from "${typeImportPath}";\n`;
}
export function generateGenericService(candidate, options = {}) {
    const { fileFormat = "Camel", classFormat = "Pascal", fieldFormat = "Camel", style = DEFAULT_COMMENT_STYLE, libraryReferenceMode, organizeByFeature = false, dirFormat, datetime, } = options;
    ...
    const generatePath = layout.filePath(candidate.name, "service");
    ...
    const servicesImport = libraryImportSpecifier("services", libraryReferenceMode, byFeature ? generatePath : `services/generated/${generatePath}`);
...
function conventionByFeaturePath(entry, names) {
...
export function resolveCustomGeneratePath(entry, names, byFeature = false) {
    const fileBase = names.casedFileStem(entry.name);
    ...
    if (byFeature) {
        ...
        // Legacy module paths (./services/* or ./routes/*) silently relocated to convention so dogfooded YAML keeps working under by-feature; the generated app.ts hands createBackendApp a customModulePaths entry so the runtime resolves to the same place while the shipped contract stays verbatim.
        ...
        if (parts[0] !== "features") {
            ...
            throw new Error(`generateCustomServiceStub: service "${entry.name}" has module "${mod}" which is outside ./features/. ` +
...
export function generateCustomServiceStub(entry, options = {}) {
    const { fileFormat = "Camel", style = DEFAULT_COMMENT_STYLE, methods = [], byFeature = false, } = options;
    ...
    const interfaceBody = methods.length === 0
        ? ""
        : methods
    ...
    const classBody = [
        ...
        ...methods.map((m) => `  async ${m}(..._args: unknown[]): Promise<unknown> {\n    return ${bodyFor(m)};\n  }`),
    ].join("\n");
    const ifaceLines = methods.length === 0
...
`;
    return { path: resolveCustomGeneratePath(entry, names, byFeature), content };
}
function indexExportLines(className, fileBase) {
    return [
        `export { ${className} } from "./${fileBase}";`,
        `export type { I${className} } from "./${fileBase}";`,
    ].join("\n");
}
function indexLineForGeneric(entityName, names) {
    return indexExportLines(names.className(entityName, "service"), names.fileBase(entityName, "service"));
}
function indexLineForCustom(entry, names) {
    return indexExportLines(entry.name, names.casedFileStem(entry.name));
}
export function generateIndexFromSchema(genericEntities, customEntries, options = {}) {
    ...
    if (Array.isArray(genericEntities) && genericEntities.length > 0) {
        const sorted = [...genericEntities].sort((a, b) => names.className(a).localeCompare(names.className(b)));
    ...
    if (Array.isArray(customEntries) && customEntries.length > 0) {
        const customDirEntries = customEntries.filter((e) => !e.module || !e.module.startsWith("."));
        if (customDirEntries.length > 0) {
            const sorted = [...customDirEntries].sort((a, b) => a.name.localeCompare(b.name));
