...
import { buildComponents, isWriteDtoViewName, } from "@deterministic-code/generator-sdk/lib/schema-build";
import { namesForSettings, layoutForSettings, } from "@deterministic-code/generator-sdk/codegen/lib/ts-codegen-naming";
...
import { refName, readBindings, bindingDatasource, resolveSelfDoc, groupRowsByEntity, } from "./frontend-bindings-routes.js";
import { CONTENT, PATCH, } from "@deterministic-code/generator-sdk/codegen/lib/emit-result";
import { BODY_METHODS, cap, fnNameOf, pathParamsOf, templatePath, } from "./client-op-model.js";
...
/** A `$ref` to a type frontend_types emits (a read entity/view) becomes that className and is added to `ctx.imports`; a `$ref` to a write-body DTO — which frontend_types does NOT emit — is inlined structurally instead of importing a symbol that wouldn't exist. The `seen` set guards against a self-referential DTO looping forever. */
function refType(ref, ctx, seen) {
    ...
    if (seen.has(raw) || !ctx.components[raw])
        return "unknown";
...
function tsTypeFor(schema, ctx, seen = new Set()) {
    if (!schema || typeof schema !== "object")
        return "unknown";
    if (schema.$ref)
        return refType(schema.$ref, ctx, seen);
...
function recordValidatorImports(ref, ctx, { withType }) {
    ctx.validatorValues.add(ref.value);
    if (withType)
        ctx.validatorTypes.add(ref.typeName);
    if (ref.needsZod)
        ctx.needsZod.add("z");
}
function operationModel(row, ctx) {
    const hasBody = BODY_METHODS.has(row.method);
    const requestType = row.request ? tsTypeFor(row.request, ctx) : null;
    const reqRef = ctx.validate && hasBody && row.request ? schemaRef(row.request, ctx) : null;
    const respRef = ctx.validate && row.response ? schemaRef(row.response, ctx) : null;
    if (reqRef)
        recordValidatorImports(reqRef, ctx, { withType: false });
    if (respRef)
        recordValidatorImports(respRef, ctx, { withType: true });
    return {
        fnName: fnNameOf(row),
        method: row.method,
...
function signature(op) {
    ...
    if (op.requestType)
        parts.push(`body: ${op.requestType}`);
    return parts.join(", ");
}
...
function importHeader(imports, typesImport) {
    if (imports.size === 0)
        return "";
...
function validatorHeader(fileCtx, validatorsImport) {
    const lines = [];
    if (fileCtx.needsZod.size > 0)
        lines.push(`import { z } from "zod";`);
    if (fileCtx.validatorValues.size > 0) {
        lines.push(`import { ${sortedList(fileCtx.validatorValues)} } from "${validatorsImport}";`);
    }
    if (fileCtx.validatorTypes.size > 0) {
        lines.push(`import type { ${sortedList(fileCtx.validatorTypes)} } from "${validatorsImport}";`);
    }
...
function fetchFn(op) {
    const init = op.requestType
        ...
        : op.method === "GET"
            ? ""
            : `, { method: "${op.method}" }`;
    const ret = op.responseType === "void" ? "" : `\n  return ${jsonResponseExpr(op)};`;
...
function tanstackHook(op) {
    return op.method === "GET" ? tanstackQuery(op) : tanstackMutation(op);
}
/** Import only the hooks the file actually declares — a query-only object omits useMutation and vice-versa — so the emitted client compiles under the frontend app's noUnusedLocals. */
function tanstackPrelude(rows) {
    const hooks = [];
...
export function renderClientFile(client, rows, ctx) {
    const fileCtx = {
        ...ctx,
        imports: new Set(),
        validatorValues: new Set(),
        validatorTypes: new Set(),
        needsZod: new Set(),
    };
    const rendered = rows.map((row) => RENDERERS[client].render(operationModel(row, fileCtx)));
    const head = [
...
/** The single settings.frontend.generate_validators flag both emits each object's colocated validators.ts and drives whether clients wrap requests/responses in `Schema.parse()` — so the client always has the schema it imports. */
function resolveValidate(settings) {
    return (settings?.frontend
        ?.generateValidators === true);
}
/** The type names frontend_types actually emits (read entities/views, not write-body DTOs) — the exact set the client may `import type` from the barrel, derived the same way frontend_types derives its output. */
function importableTypeNames(viewData, datasourceData, names) {
...
function clientFileEntries(ds, ctx, rows) {
    const entries = [];
    for (const [entity, entityRows] of groupRowsByEntity(rows)) {
        ...
        for (const client of ds.clients) {
            const clientFile = ctx.layout.frontendClientFile(ds.name, entity, `${client}.ts`);
            entries.push({
                kind: CONTENT,
                filename: clientFile,
                contents: renderClientFile(client, entityRows, {
                    ...fileCtx,
                    validatorsImport: ctx.layout.frontendRelImport(clientFile, validatorFile),
                }),
            });
        }
    }
...
function dependencyPatch(clients) {
    const dependencies = {};
...
async function datasourceEntries(entry, base) {
    const ds = bindingDatasource(entry);
    const clients = ds.clients.filter((c) => CLIENT_LIBS.has(c));
    if (clients.length === 0)
...
/** The read-only ctx every datasource's client files share: resolved names/layout/validate/datetime plus the set of type names frontend_types actually emits (so a `$ref` to one imports rather than inlines). */
async function buildBaseCtx(inputs, settings) {
    const names = namesForSettings(settings, "typescript");
    const { viewYamlText, datasourceYamlText } = await inputs.all();
    const importable = importableTypeNames(parse(viewYamlText), datasourceYamlText
...
export async function emit({ inputs, settings, }) {
    ...
    for (const entry of datasources) {
        ...
        for (const client of built.clients)
