...
import { routeServiceImport, routeValidatorImport, } from "./emit-routes-typescript.js";
...
function nestedEmitPath(descriptor, options, byFeature) {
...
function nestedRouterPreamble(descriptor, options) {
    ...
    const routesImport = libraryImportSpecifier("routes", options.libraryReferenceMode, byFeature ? emitPath : `routes/generated/${fileBase}.ts`);
...
function nestedDirectFkContent(descriptor, options) {
    ...
    const repositoriesImport = libraryImportSpecifier("repositories", opts.libraryReferenceMode, nestedEmitPath(descriptor, options, options.organizeByFeature === true));
...
export function emitNestedDirectFkRouter(descriptor, options = {}) {
    if (descriptor.kind !== "direct-fk") {
        throw new Error(`emitNestedDirectFkRouter: expected direct-fk descriptor, got ${descriptor.kind}`);
    }
    const byFeature = options.organizeByFeature === true;
    return {
...
export function emitNestedM2mRouter(descriptor, options = {}) {
    if (descriptor.kind !== "m2m") {
        throw new Error(`emitNestedM2mRouter: expected m2m descriptor, got ${descriptor.kind}`);
    }
    const byFeature = options.organizeByFeature === true;
    return {
        path: nestedEmitPath(descriptor, options, byFeature),
