...
import { FRONTEND_SERVICE, FRONTEND_PORT, } from "@deterministic-code/generator-sdk/codegen/lib/compose-services";
...
/** The single supported {language: framework} pair. framework already seeds React (typescript, Frontend tier) in datasource_types.yaml; this guard keeps the emitter honest about what it can actually scaffold. */
const SUPPORTED_FRAMEWORK_BY_LANGUAGE = {
    typescript: "React",
};
/** Each template file → its path under the emitted frontend/ subtree. Only index.html/App.tsx carry the {{appName}} token; the rest render verbatim. .gitignore and package.json are NOT here — each emits as a PATCH: .gitignore via the shared-append writer, package.json as the skeleton piece for the deep-merge writer so a later frontend-lane step (e.g. client_bindings) can add a dependency without last-writer-wins clobbering the manifest. */
const TEMPLATE_TO_OUTPUT = [
...
function renderFrontendComposeService() {
...
function assertSupported(language, framework) {
    if (SUPPORTED_FRAMEWORK_BY_LANGUAGE[language] !== framework) {
        throw new Error(`frontend_app: unsupported { language: ${language}, framework: ${framework} } — only { typescript, React } is supported.`);
    }
}
...
export async function emit({ settings, args }) {
    const language = args?.language ?? DEFAULT_LANGUAGE;
    const framework = args?.framework ?? settings?.frontend?.framework ?? DEFAULT_FRAMEWORK;
