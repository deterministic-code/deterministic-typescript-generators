...
export function joinImport(base, file) {
    if (base === ".")
        return `./${file}`;
...
export function libraryImportSpecifier(subpath, mode, emittedFileRelToProjectRoot) {
    if (mode !== "bundled") {
        return subpath ? `${NPM_PACKAGE}/${subpath}` : NPM_PACKAGE;
    }
    const dir = posix.dirname(emittedFileRelToProjectRoot);
    let rel = posix.relative(dir, "_deterministic");
    if (!rel)
        rel = ".";
    if (!rel.startsWith("."))
        rel = `./${rel}`;
    // why explicit .js for bundled: tsc with moduleResolution=bundler preserves the import string verbatim into the compiled JS; Node ESM at runtime requires an explicit extension or a package.json exports map — bundled `_deterministic/` ships neither, so the extension goes on at emit-time. Missing it triggered ERR_MODULE_NOT_FOUND on `/app/dist/_deterministic/app` even after PR #1122's runtime-stage COPY landed.
    return subpath ? `${rel}/${subpath}.js` : `${rel}/index.js`;
}
const LIBRARY_IMPORT_RE = new RegExp(`(["'])${NPM_PACKAGE.replace(/[/\\]/g, "\\$&")}(?:/([\\w./-]+))?\\1`, "g");
...
export function rewriteLibraryImports(source, mode, emittedFileRelToProjectRoot) {
    return source.replace(LIBRARY_IMPORT_RE, (_match, quote, subpath) => `${quote}${libraryImportSpecifier(subpath ?? "", mode, emittedFileRelToProjectRoot)}${quote}`);
}
