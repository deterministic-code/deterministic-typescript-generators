...
export function libraryImportSpecifier(subpath, mode, generatedFileRelToProjectRoot) {
    if (mode !== "bundled") {
...
export function rewriteLibraryImports(source, mode, generatedFileRelToProjectRoot) {
    return source.replace(LIBRARY_IMPORT_RE, (_match, quote, subpath) => `${quote}${libraryImportSpecifier(subpath ?? "", mode, generatedFileRelToProjectRoot)}${quote}`);
}
