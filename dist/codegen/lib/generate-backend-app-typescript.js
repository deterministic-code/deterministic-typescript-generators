...
import { resolveCustomGeneratePath } from "./generate-services-typescript.js";
import { resolveCustomRoutePath } from "./generate-routes-typescript.js";
import { generateTestApp } from "./generate-test-app-typescript.js";
...
function toSpecifier(generatePath) {
...
export function buildCustomModulePaths({ servicesDoc, routesDoc, names, }) {
    ...
    for (const entry of asArray(servicesDoc?.services)) {
        ...
        if (mod) {
            const path = resolveCustomGeneratePath(entry, names, true);
