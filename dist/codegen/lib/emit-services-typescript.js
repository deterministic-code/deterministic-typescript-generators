...
export function emitGenericService(candidate, options = {}) {
    ...
    const servicesImport = libraryImportSpecifier("services", libraryReferenceMode, byFeature ? emitPath : `services/generated/${emitPath}`);
...
export function resolveCustomEmitPath(entry, names, byFeature = false) {
    const fileBase = names.casedFileStem(entry.name);
    ...
    if (byFeature) {
        ...
        if (parts[0] !== "features") {
            ...
            throw new Error(`emitCustomServiceStub: service "${entry.name}" has module "${mod}" which is outside ./features/. ` +
...
export function emitIndexFromSchema(genericEntities, customEntries, options = {}) {
