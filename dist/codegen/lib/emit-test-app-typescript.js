...
function assertObjectDoc(name, data) {
    if (!data || typeof data !== "object") {
        throw new Error(`emitTestApp: ${name} is required`);
    }
}
...
function renderStubClass(service) {
    const methodLines = service.methods.map((m) => {
        const returnLiteral = m.sampleResponse === null ? "null" : JSON.stringify(m.sampleResponse);
...
function renderServiceSpecsLiteral(serviceNames) {
    if (serviceNames.length === 0)
        return "[]";
    const entries = serviceNames.map((name) => `    { name: "${name}", args: [] }`);
    return `[\n${entries.join(",\n")},\n  ]`;
}
function renderClassRegistryEntries(customServices) {
    const stubLines = customServices.map((svc) => `      ${svc.serviceName}: ${svc.stubClassName} as unknown as new (...a: unknown[]) => unknown,`);
    return stubLines.join("\n");
}
export function emitTestApp({ datasourceData, routesData, viewTypesData, factoryName = "createTestApp", pluralizeTableNames = true, datetime = "native", uuid = "native", idType = "integer", libraryReferenceMode, organizeByFeature = false, }) {
    ...
    const routeSpecsExpr = serviceNames.length > 0 ? "parseGenericRouteSpecs(routesData)" : "[]";
    const parseGenericImport = serviceNames.length > 0 ? "\n  parseGenericRouteSpecs," : "";
    const detRootImport = libraryImportSpecifier("", libraryReferenceMode, "__tests__/test-app.ts");
    const middlewareImport = libraryImportSpecifier("middleware", libraryReferenceMode, "__tests__/test-app.ts");
...
export async function ${factoryName}(): Promise<Express> {
  ...
  const app = await createBackendApp(conn, {
    ...
    // No app-level seed loop — emitted _initial_up.sql already INSERTs every \`seeds:\` row, so re-seeding here would UNIQUE-violate (GATE 11). For test-only fixtures outside the production schema, emit an \`onAfterCreateApp\` hook instead.
  });
