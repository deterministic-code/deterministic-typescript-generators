...
import { nestedRouterFileBase, nestedRouterFnName, nestedRouteEntity, descriptorFileFormat, nestedMountPath, } from "./emit-nested-routes-typescript.js";
...
export function emitNestedDirectFkRouterTest(descriptor, options = {}) {
    if (descriptor.kind !== "direct-fk") {
        throw new Error(`emitNestedDirectFkRouterTest: expected direct-fk descriptor, got ${descriptor.kind}`);
    }
    return {
...
export function emitNestedM2mRouterTest(descriptor, options = {}) {
    if (descriptor.kind !== "m2m") {
        throw new Error(`emitNestedM2mRouterTest: expected m2m descriptor, got ${descriptor.kind}`);
    }
    return {
        path: nestedRouterTestPath(descriptor, options),
