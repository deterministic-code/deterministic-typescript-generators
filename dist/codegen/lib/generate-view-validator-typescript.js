...
import { datetimeMergeGenerator, validatorOptionsFromSettings, } from "@deterministic-code/generator-sdk/codegen/lib/generate-settings-options";
...
function buildImports(view, ctx) {
    ...
    const add = (sortKey, generatePath, token) => {
...
function generateShapedSchema(view, ctx) {
...
function generateUnionSchema(view) {
    const schemaName = viewSchemaIdent(view.name);
...
function renderView(view, ctx) {
    ...
    const schemaBody = view.kind === "union" ? generateUnionSchema(view) : generateShapedSchema(view, ctx);
...
export const createGenerator = () => {
    const base = datetimeMergeGenerator(baseCreateGenerator, TypescriptImports, DEFAULT_GENERATE_OPTIONS);
    return {
