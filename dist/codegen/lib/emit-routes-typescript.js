...
import { emitNestedDirectFkRouter, emitNestedM2mRouter, } from "./emit-nested-routes-typescript.js";
...
function byFeatureRoutePath({ entry, names, name, mod, }) {
    ...
    if (parts[0] !== "features") {
        ...
        throw new Error(`emitCustomRouteStub: route "${name}" has module "${mod}" which is outside ./features/. ` +
...
export function emitCustomRouteStub(entry, options = {}) {
...
export function emitIndexFromFiles(files) {
    const indexFiles = [];
