...
import { converterTypeForSchema } from "@deterministic-code/generator-sdk/lib/schema-build";
import { NUMERIC_TYPES } from "@deterministic-code/generator-sdk/lib/field-converter";
const REF_PREFIX = "#/components/schemas/";
function refName(ref) {
    return ref.slice(REF_PREFIX.length);
}
function scalarSample(schema, datetime) {
    ...
    if (key === "date")
        return new RuntimeValue("date");
    if (key === "binary")
        return new Uint8Array(0);
    if (key === "email")
        return new RuntimeValue("email");
...
function baseSample(schema, ctx, seen) {
...
function objectSample(schema, ctx, seen) {
    if (!schema || typeof schema !== "object")
        return null;
...
export function sampleForSchema(schema, components, { datetime = "string", ident = IDENTITY } = {}) {
...
export function sampleForComponent(name, components, { datetime = "string", ident = IDENTITY } = {}) {
    ...
    return objectSample(schema, { components: comps, datetime, ident, nullableVariant: false }, new Set([name]));
}
...
export function nullableVariantForComponent(name, components, { datetime = "string", ident = IDENTITY } = {}) {
    const comps = components;
    const schema = comps[name];
    if (!schema)
...
export function nullableFieldNames(name, components, { ident = IDENTITY } = {}) {
    ...
    return Object.keys(props)
        .filter((key) => props[key].nullable === true && props[key].type !== "array")
        .map(ident);
}
function isPlainScalar(prop) {
    return (!prop.$ref &&
        !prop.oneOf &&
        prop.type !== "array" &&
        (prop.type === "integer" ||
...
export function enumerateComponentMutations(name, components, { ident = IDENTITY } = {}) {
...
export function scalarSetFields(name, components, { ident = IDENTITY } = {}) {
    const props = components[name]?.properties ?? {};
    return Object.entries(props)
        .filter(([, prop]) => isPlainScalar(prop) &&
        prop.nullable !== true &&
